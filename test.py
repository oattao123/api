...
class DatabaseManager:
    ...
    def update_cryptocurrency(self, user_id, crypto_name, new_quantity):
        try:
            self.cursor.execute("UPDATE cryptocurrencies SET crypto_quantity=%s WHERE user_id=%s AND crypto_name=%s", (new_quantity, user_id, crypto_name))
            self.connection.commit()
        except Exception as e:
